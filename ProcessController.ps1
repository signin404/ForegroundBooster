...
# 步骤 0: 定义脚本参数 以支持不同的启动模式 (移除验证以允许-1)
param(
    [string]$ProcessName,
    [int]$ProcessId,
    [string]$Affinity,
...
function Parse-AffinityString {
    ...
    if ($InputString -eq "-1") { return $null }
    [uint64]$totalMask = 0
...
$jobObjectName = "Global\ProcessControllerJob_$($jobId)"
...
if ($isOneShotMode) {
    ...
    if ($PSBoundParameters.ContainsKey('Affinity')) { if ($Affinity -ne "-1") { $settings.Affinity = Parse-AffinityString -InputString $Affinity } }
    if ($PSBoundParameters.ContainsKey('Scheduling')) { if ($Scheduling -ne -1) { if ($Scheduling -in 0..9) { $settings.Scheduling = $Scheduling } else { Write-Warning "调度优先级 '$Scheduling' 无效. 必须在 0-9 之间." } } }
    if ($PSBoundParameters.ContainsKey('Priority')) { if ($Priority -ne "-1") { if ([System.Enum]::IsDefined([NativeMethods+PROCESS_PRIORITY_CLASS], $Priority)) { $settings.Priority = $Priority } else { Write-Warning "优先级 '$Priority' 无效." } } }
    if ($PSBoundParameters.ContainsKey('Working')) { if ($Working -ne "-1") { $settings.WorkingSet = Parse-WorkingSetString -InputString $Working } }
    if ($PSBoundParameters.ContainsKey('Weight')) { if ($Weight -ne -1) { if ($Weight -in 1..9) { $settings.Weight = $Weight } else { Write-Warning "时间片权重 '$Weight' 无效. 必须在 1-9 之间." } } }
    if ($PSBoundParameters.ContainsKey('CpuLimit')) { if ($CpuLimit -ne -1) { if ($CpuLimit -in 1..100) { $settings.CpuLimit = $CpuLimit } else { Write-Warning "CPU使用率限制 '$CpuLimit' 无效. 必须在 1-100 之间." } } }
    if ($PSBoundParameters.ContainsKey('NetLimit')) { if ($NetLimit -ne -1) { if ($NetLimit -gt 0) { $settings.NetLimit = $NetLimit } else { Write-Warning "传出带宽限制 '$NetLimit' 无效. 必须大于 0." } } }
    if ($PSBoundParameters.ContainsKey('DSCP')) { if ($DSCP -ne -1) { if ($DSCP -in 0..63) { $settings.DSCP = $DSCP } else { Write-Warning "数据包优先级 '$DSCP' 无效. 必须在 0-63 之间." } } }
    ...
    try {
        ...
        $currentSettings = @{
            Affinity   = if ($status.Affinity -ne '已禁用') { [Convert]::ToUInt64($status.Affinity.Substring(2), 16) } else { $null }
            Priority   = if ($status.Priority -ne '已禁用') { $status.Priority } else { $null }
            Scheduling = if ($status.Scheduling -ne '已禁用') { [int]$status.Scheduling } else { $null }
            Weight     = if ($status.Weight -ne '已禁用') { [int]$status.Weight } else { $null }
            CpuLimit   = if ($status.CpuLimit -ne '已禁用') { [int]($status.CpuLimit -replace '%', '') } else { $null }
            NetLimit   = if ($status.NetLimit -ne '已禁用') { [int]($status.NetLimit -replace ' KB/s', '') } else { $null }
            DSCP       = if ($status.DSCP -ne '已禁用') { [int]$status.DSCP } else { $null }
            WorkingSet = if ($status.WorkingSet -ne '已禁用') { Parse-WorkingSetString -InputString ($status.WorkingSet -replace 'MB', '') } else { $null }
        }
        ...
        while ($true) {
            ...
            switch ($choice) {
                '1' {
                    $input = Read-Host "新亲和性 (例: 8 10 12-15) 或 -1 禁用"
                    if ($input -eq '-1') { $currentSettings.Affinity = $null }
                    else {
                ...
                '2' {
                    $input = Read-Host "新优先级 (Idle, BelowNormal, Normal, AboveNormal, High, RealTime) 或 -1 禁用"
                    if ($input -eq '-1') { $currentSettings.Priority = $null }
                    elseif ([System.Enum]::IsDefined([NativeMethods+PROCESS_PRIORITY_CLASS], $input)) { $currentSettings.Priority = $input } 
                    else { Write-Warning "无效输入" }
                }
                '3' {
                    $input = Read-Host "新调度优先级 (0-9) 或 -1 禁用"
                    if ($input -eq '-1') { $currentSettings.Scheduling = $null }
                    elseif ($input -match '^[0-9]$') { $currentSettings.Scheduling = [int]$input } 
                    else { Write-Warning "无效输入" }
                }
                '4' {
                    $input = Read-Host "新时间片权重 (1-9) 或 -1 禁用"
                    if ($input -eq '-1') { $currentSettings.Weight = $null }
                    elseif ($input -match '^[1-9]$') { $currentSettings.Weight = [int]$input; $currentSettings.CpuLimit = $null } 
                    else { Write-Warning "无效输入" }
                }
                '5' {
                    $input = Read-Host "新数据包优先级 (0-63) 或 -1 禁用"
                    if ($input -eq '-1') { $currentSettings.DSCP = $null }
                    elseif ($input -match '^\d+$' -and [int]$input -ge 0 -and [int]$input -le 63) { $currentSettings.DSCP = [int]$input } 
                    else { Write-Warning "无效输入" }
                }
                '6' {
                    $input = Read-Host "新CPU使用率上限 (1-100) 或 -1 禁用"
                    if ($input -eq '-1') { $currentSettings.CpuLimit = $null }
                    elseif ($input -match '^\d+$' -and [int]$input -ge 1 -and [int]$input -le 100) { $currentSettings.CpuLimit = [int]$input; $currentSettings.Weight = $null } 
                    else { Write-Warning "无效输入" }
                }
                '7' {
                    $input = Read-Host "新传出带宽上限 (KB/s) 或 -1 禁用"
                    if ($input -eq '-1') { $currentSettings.NetLimit = $null }
                    elseif ($input -match '^\d+$' -and [int]$input -gt 0) { $currentSettings.NetLimit = [int]$input } 
                    else { Write-Warning "无效输入" }
                }
                '8' {
                    $input = Read-Host "新物理内存上限 (格式: 最小MB-最大MB) 或 -1 禁用"
                    if ($input -eq '-1') { $currentSettings.WorkingSet = $null }
                    else {
